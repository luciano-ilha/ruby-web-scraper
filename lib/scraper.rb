class Scraper
  attr_reader :series_title, :series_start_date, :series_meter_score, :series_countdown_index, :series_cast, :series_synopsis_links
  ...
  def series_info
    @series_list.each do |item|
      ...
      @series_cast << item.css('div.cast').text.strip.to_s
  ...
  def invert_series_info
    ...
    @series_cast = series_cast.reverse
